...
def configurar_bd_temporal():
    """Ruta definitiva para insertar el admin y las formas de pago correctas en Render"""
    try:
        # 1. Insertar formas de pago si la tabla está vacía usando 'desformapago'
        if FormaPago.query.count() == 0:
            efectivo = FormaPago(desformapago="Efectivo", estado=1)
            tarjeta = FormaPago(desformapago="Tarjeta de Débito/Crédito", estado=1)
            qr = FormaPago(desformapago="Código QR", estado=1)
